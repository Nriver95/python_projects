...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) >0) and (len(var_phone) > 0) and(len(var_email) > 0): #enforce the user to probide both names
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
        ...
        messagebox.showerror("Missing Text Error","Please ensure that there is a data in all four fields.")
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        #check count ot ensure that this is not the last record in the database.
        #One cannot delete the last record or en error will occur
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
#####               onRefresh(self) # update the listbox of the changes
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the database and cannot be deleted at this time. \n\nPlease add another record first before youi can delete ({}).".format(var_select,var_select))
    conn.close()
